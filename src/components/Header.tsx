...
function Header() {
  ...
  // const isHome = useMatch("/newflix");
  const isTv = useMatch({
    path: "/newflix/tv",
    end: false
  });
  const isSearch = useMatch({
    path: "/newflix/search",
    end: false
  })
  ...
  const onValid = (data:IForm) => {
    navigate(`/newflix/search?keyword=${data.keyword}`);
    setValue("keyword","");
    setSearch(false);
    return;
  }
  ...
  return (
    <Boxes ref={scope}>
      <Box>
        ...
        <Menus>
          <Menu key="home"><Link to="/newflix">Home</Link> 
          </Menu>
          <Menu key="tv"><Link to="/newflix/tv">Tv Shows</Link> 
          </Menu>
          <AnimatePresence initial={false}>
            {isHome && !isTv && !isSearch ? <HomeSelected key="home" layoutId="selected"
            initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}/> : null}
            {isTv && !isSearch ? <TvSelected key="tv" layoutId="selected"
            initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}/> : null}
          </AnimatePresence>
        </Menus>
      </Box>
